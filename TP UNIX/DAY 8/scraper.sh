...
analysis()
{
	...
	then
		...
		n=`grep -E '[0-99]' n.txt`
		rm -f n.txt
		printf "analysing %s movies from allocine..\n" "$n"
	...
	then
		...
		n=`grep -E '[0-99]' n.txt`
		rm -f n.txt
		printf "analysing %s movies from premiere..\n" "$n"

		grep -E "tabindex=\"0\">Bandes-annonces</a>" data_premiere.txt > ana_tra_prem.txt
		sed -e 's+<a\ href="+http://www.premiere.fr+g' -i ana_tra_prem.txt
		sed -e 's+"\ tabindex="0">Bandes-annonces</a>++g' -i ana_tra_prem.txt
		...
		paste ana_tit_prem.txt ana_tra_prem.txt ana_typ_prem.txt ana_rat_prem.txt | while IFS="$(printf '\t')" read -r f1 f2 f3 f4
		do
			printf "Titre : %s\n" "$f1"
			printf "%s\n" "$f2"
			printf "	genre : %s\n" "$f3"
	...
	then
		...
		n=`grep -E '[0-99]' n.txt`
		rm -f n.txt
		printf "analysing %s movies from telerama..\n" "$n"
	...
	then
		...
		n=`grep -E '[0-99]' n.txt`
		rm -f n.txt
		printf "analysing %s movies from senscritique..\n" "$n"
...
web()
{
	...
	while ((i!=$n+1))
		do
			...
			grep -i -E -A4 "$t" ana_allocine.txt > tmp$i.txt
			j=2
			...
			grep -i -E -A3 "$t" ana_premiere.txt > tmp$i.txt
			j=2
			...
			grep -i -E -A2 "$t" ana_telerama.txt > tmp$i.txt
			j=2
			tt1=`head -$j tmp$i.txt | tail -1`
			((j++))
			tt2=`head -$j tmp$i.txt | tail -1`

			grep -i -E -A4 "$t" ana_senscritique.txt > tmp$i.txt
			j=2
			...
			printf "<ul>" >> $2
			printf "<a href='%s'><img src='%s' /></a></li>" "$tp1" "$ta1" >> $2 #bande annonce dans l'image
			printf "<li>%s</li>" "$g" >> $2 #genre
			printf "<li>%s</li>" "$ta2" >> $2 #synopsis
			printf "<li>%s</li>" "$ts2" >> $2 #réal
			printf "<li>%s</li>" "$tt1" >> $2 #acteurs/roles
			printf "<li>%s</li>" "$ts3" >> $2 #durée
			printf "<li>%s</li>" "$ts1" >> $2 #sortie
			printf "<li>notes : </li>" >> $2
			printf "<ul>" >> $2
			printf "<li>_%s/5</li>" "$ta3" >> $2 #note spec
			printf "<li>_%s/5</li>" "$ta4" >> $2 #note presse
			printf "<li>_%s/5</li>" "$tp3" >> $2 #note
			printf "<li>_%s/10</li>" "$ts4" >> $2 #note
			printf "</ul>" >> $2
			printf "</ul>" >> $2
			((i++))		
		done
...
error()
{
	echo "enter a valid argument, here is the -h page :"
	./scraper.sh -h
}

remove()
{
	echo "removing html pages.."
	rm -f *.html
	echo "pages removed"
}
