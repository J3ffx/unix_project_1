	...
	echo "starting curl.."
		...
		then
			...
			then
				...
				curl -a firefox $n > data_allocine.txt
				echo "curling :: $n"
				rm site.txt
			fi
		fi
		...
		then
			...
			then
				...
				curl -a firefox $n > data_premiere.txt
				echo "curling :: $n"
				rm site.txt
			fi
		fi
		...
		then
				...
				while true ; do
					...
					curl -a firefox $lnk_p > data_temp.txt
...
analysis()
{
	...
	then
		...
		n=`grep -E '[0-99]' n.txt`
		rm n.txt
		printf "analysing %s movies from snescritique..\n" "$n"
...
web()
{
	...
	if [ -e ana_allocine.txt ]
	then
		cp ana_allocine.txt temp.txt
		sed -i '1s/^/--\n/' data_timt.txt
