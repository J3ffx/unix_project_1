...
{	
		...
		if [ "$2" = 'telerama' -o "$2" = 'tele' -o "$2" = '3' -o "$2" = '-' ]
		then
				...
				while true ; do
					...
					curl --silent -a firefox $lnk_p > data_temp.txt
		...
		if [ "$2" = 'senscritique' -o "$2" = 'sens' -o "$2" = '4' -o "$2" = '-' ]
		then
...
locald()
{	
		sed -e 's+http://+htpp://localhost/+g' -i sitelist.txt && sed -e 's+https://+htpp://localhost/+g' -i sitelist.txt
		download $1 $2;
}
...
web()
{
	...
	rm -f temp.txt
	uniq -i -w 6 tmp.txt > web_tit.txt
	rm -f tmp.txt
